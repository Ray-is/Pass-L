#Author - https://github.com/danjperron/micropython-mfrc522
...
class MFRC522:
    ...
    def MFRC522_DumpClassic1K(self, uid, Start=0, End=64, keyA=None, keyB=None):
        for absoluteBlock in range(Start,End):
            ...
            print("{:02d} S{:02d} B{:1d}: ".format(absoluteBlock, absoluteBlock//4 , absoluteBlock % 4))
            
            # This code only runs if auth was successful.
            if status == self.OK:
                status, block = self.read(absoluteBlock)
                
                # This only runs if there was an error reading from the RFID keyfob.
                if status == self.ERR:
                    #print("ERR")
                    break
                
                # Does nothing; just prints
                else:
                    for value in block:
            ...
            # This code runs when the auth was unsuccessful.
            else:
                break
...
if __name__ == "__main__":
    ...
    SAVED_UID = "[0x6F, 0xF3, 0x86, 0xC2]"
    while True:

        reader.init()
        (stat, tag_type) = reader.request(reader.REQIDL)
        if stat != reader.OK:
            print("RFID fob not present")
            continue
    
        (stat, uid) = reader.SelectTagSN()
        if stat != reader.OK:
            print("ERROR reading RFID fob.")
            continue
        print(f"Read card with UID: {reader.tohexstring(uid)}")
        if reader.tohexstring(uid) != SAVED_UID:
            print("Access denied.")
        else:
